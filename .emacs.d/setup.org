...
#+BEGIN_SRC emacs-lisp
  (customize-set-variable
   ...
   '(benchmark-init
     dash dash-functional s f bind-key delight suggest
     ...
     drag-stuff zzz-to-char fix-word hungry-delete aggressive-indent
     clojure-mode cider clj-refactor kibit-helper
...
#+BEGIN_SRC emacs-lisp
  (unbind-keys emacs-lisp-mode-map "C-M-i")
  (unbind-keys lisp-interaction-mode-map "C-M-i" "C-j")
  (bind-keys :map lisp-interaction-mode-map ("RET" . eval-print-last-sexp))
...
** editing
*** structure-based
with revised symmetrical key chords.
|   | C        | M        | C-M      |
|---+----------+----------+----------|
