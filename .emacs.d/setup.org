...
#+BEGIN_SRC emacs-lisp
  (customize-set-variable
   ...
   '(benchmark-init
     ...
     exec-path-from-shell rime
     zenburn-theme powerline rainbow-delimiters unicode-fonts
     phi-search selectrum prescient selectrum-prescient which-key
     centered-cursor-mode vimish-fold ace-window avy
     projectile magit undo-tree deadgrep
     smartparens expand-region region-bindings-mode multiple-cursors
     ...
     geiser geiser-chez ediprolog pasp-mode haskell-mode idris-mode
...
#+BEGIN_SRC emacs-lisp
  ...
  (with-eval-after-load 'tramp-cache
    (push '(".*" "locale" "LC_ALL=C") tramp-connection-properties))
#+END_SRC
for chinese input.
#+BEGIN_SRC emacs-lisp
  (cset 'rime-disable-predicates '(rime-predicate-current-uppercase-letter-p
...
*** search
#+BEGIN_SRC emacs-lisp
  (bind-keys ("C-s" . phi-search)          ("C-M-s" . isearch-forward)
             ("C-r" . phi-search-backward) ("C-M-r" . isearch-backward)
             ("M-%" . phi-replace-query)   ("C-M-%" . query-replace))
#+END_SRC
*** symbol
#+BEGIN_SRC emacs-lisp
  (selectrum-mode 1)
...
#+BEGIN_SRC emacs-lisp
  ...
  (cset 'undo-tree-auto-save-history nil)
#+END_SRC
** editing
*** structure-based
...
#+BEGIN_SRC emacs-lisp
  ...
  (set-face-attribute 'sp-show-pair-mismatch-face nil :background "#161616" :foreground "#003B6F" :weight 'bold)
#+END_SRC
*** region-based
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  (let* ((coding-system-for-read 'utf-8)
    ...
    (when (s-ends-with? "agda2.el" agda-mode-path)
      (load-file agda-mode-path)
      (push '("\\.lagda.md\\'" . agda2-mode) auto-mode-alist)
      (push 'agda2-mode sp-no-reindent-after-kill-modes)
      (with-eval-after-load 'agda2-mode
        (def-eval-dwim eval-dwim:agda2 agda2-load agda2-goal-and-context)
        (bind-keys :map agda2-mode-map ("M-RET" . eval-dwim:agda2)))
      (with-eval-after-load 'agda2-highlight
        (--each '(agda2-highlight-error-face
...
| %       | []                                   | phi-replace-query                                            | query-replace                                                |
| ^       |                                      | delete-indentation                                           |                                                              |
| &       | []                                   | async-shell-command                                          |                                                              |
| *       | []                                   | ~sp-convolute-sexp~                                          |                                                              |
...
| r       | phi-search-backward                  | move-to-window-line-top-bottom                               | isearch-backward                                             |
| s       | phi-search                           | [M-s-]                                                       | isearch-forward                                              |
